...
public class Message0x08LinkUpdateVisitor extends ClassVisitor {
	private static class HookMethodVisitor extends MethodVisitor {
		...
		public void visitInsn(final int opcode) {
			if (opcode==Opcodes.RETURN) {
				...
				visitMethodInsn(Opcodes.INVOKESTATIC, "net/teamfruit/visualink/addons/jabba/MessageLinkUpdateHook", "channelRead0x08",
	...
	public Message0x08LinkUpdateVisitor(final @Nonnull String obfClassName, final @Nonnull ClassVisitor cv) {
		super(Opcodes.ASM5, cv);
		this.matcher = new MethodMatcher(VisitorHelper.getMappedName("mcp/mobius/betterbarrels/network/Message0x08LinkUpdate"),
				DescHelper.toDescMethod(void.class, "io.netty.channel.ChannelHandlerContext", "mcp.mobius.betterbarrels.network.Message0x08LinkUpdate"), RefName.name("channelRead0"));
	}
